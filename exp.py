...
def adaptive_canny(img, target=0.045, lo_ratio=0.5, iters=12):
    ...
    ★注意: 目標に「最も近い」地図を採るだけなので、密度が階段状に変わる
    入力では空の地図（密度0）が選ばれうる。実際 synth_veryd（α=0.3）は
    0.1316 から 0.0 へ落ちる。|0-0.045| < |0.1316-0.045| だからである。
    第6節が推奨する形では、改善前の密度が目標を下回るときにだけ適用するので
    この入力は対象から外れる。無条件に使うなら密度0を弾く判定が要る。
    """
