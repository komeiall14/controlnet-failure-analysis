...
**本レポートで示したこと。** ControlNet の制御が壊れる条件を 3 つ切り出し、いずれも「なぜ壊れるか」まで実験で示した。推奨されている省メモリ設定が黙って出力を壊す件は、`get_attention_scores` が未初期化のまま渡す加算元に原因を絞り込み、そこを 0 で埋めるだけで直ることを確かめた。条件地図の密度が下がると制御が消える件は、指定した輪郭との一致が 0.7329 から 0.2391 へ落ちて偶然の水準と区別できなくなる一方、生成画像自体は破綻しない。UNet へ注入される残差を直接測ると密度とともに縮んでおり（密度が正の領域で ρ=+0.669）、**制御が消えるのは残差が消えるからではなく、条件地図に由来する成分が縮むからである。** ここから密度を揃える前処理を作り、構造整合が改善してプロンプト追従は下がらないことを確かめた。三つ目は推論時の既定値が構造整合に対して弱いという件で、これは破綻ではなく二つの指標のあいだの妥協点として第 8 節で扱う。
...
`beta = 0` なので加算元の中身は結果に影響しないはずで、BLAS の規約でも beta = 0 のとき加算元は参照されないと定められている。未初期化のまま渡す実装自体は規約に従ったものである。溢れの線も薄い。入力の絶対値の最大は 8.6 で、内積を 160 次元ぶん蓄積して係数を掛けても fp16 の上限 65504 には 3 桁届かない。

決め手は加算元の差し替えである。**`torch.empty` を `torch.zeros` に変えるだけで NaN が消えた。** 粒度 1・4・auto のいずれでも消え、潜在変数の絶対値の最大は 3.379〜3.383 と、slicing 無効時の 3.377 に近い値へ戻る（経路が違うので厳密には一致しない）。シードを変えても消えたままだった。**したがって加算元の中身が結果に伝播している。** 規約どおりなら伝播しないはずなので、MPS の `baddbmm` が beta = 0 でも加算元を読んでいることになる。IEEE 754 では 0 × NaN も 0 × Inf も NaN なので、非有限のビット列が残っていれば出力全体が汚染される。**ただしバッファの中身そのものは観測していないので、汚染が非有限値によるものかまでは確定できない。** fp32 で露見しない理由も分かっていない。
...
**指標の設計に依存している。** 構造整合は再抽出した Canny との F1 で測るが、2 画素の許容幅・再抽出時の閾値・膨張マッチングに依存し、許容幅を変えれば数値は動く。条件地図が空だと定義から 0 を返すので、破綻そのものをこの指標で測ることはできない。代替に用いた偶然一致水準との比較も、「本来指定したかった輪郭」を人為的に選んでいる。
