...
だから最適値は入力の中にあるのではなく、入力の構造と、何を達成したいかの組み合わせの中にある。
...
数値はすべて自分の手元の実測データから再計算して確認している。批判的レビューの過程で生成AI が測定値を提示した箇所もあったが、その値を採るにあたっては同じ測定を自分で実装し（第 5 節の残差の空間分解は `probe_spatial.py`）、再現を確認したうえで本文に載せている。再現できなかった数値は本文から外した。
...
[4] diffusers v0.31.0 実装（`models/controlnet.py`、`models/attention_processor.py`）。重み付けが `guess_mode` 有効時のみ適用されること、slicing で attention の経路が入れ替わることの確認に使用した。

[5] Stéfan van der Walt, Johannes L. Schönberger, Juan Nunez-Iglesias, François Boulogne, Joshua D. Warner, Neil Yager, Emmanuelle Gouillart, Tony Yu, and the scikit-image contributors. "scikit-image: image processing in Python." *PeerJ* 2:e453, 2014. DOI: 10.7717/peerj.453 （入力画像に同梱データを使用。帰属は LICENSE_DATA.md）

書誌情報は [1] [2] [5] を Crossref で DOI 照合し、[1] は DBLP と Semantic Scholar でも確認した（DBLP 上で arXiv 版と会議録版が別項目として登録されており、本会議採録である）。[1] の supplementary は CVF の公開版を取得して参照した。
