...
これは上流の実装の問題なので、修正も 1 行で済む（本レポートの改善その 1）。fp32 に切り替えても回避できるが、同じ 4 ステップで fp16 が 23.6 秒、fp32 が 39.9 秒と **1.7 倍**遅くなる（`bench_dtype.py`。ウォームアップ 1 回を捨てた 3 回の中央値。上の格子は 1 条件 1 回で初期化を被るため速度には使えない）。以降の実験は fp16 のまま slicing を無効にして行った。
...
改善の 2 つ目である。1 つ目は第 4.1 節で述べた未初期化バッファの修正で、こちらは推論の前段が静かに壊れる問題を直すものだった。本節が扱うのは制御そのものの品質である。第 4.2 節と第 5 節から、条件が効かなくなる原因は閾値が固定されていることに帰着する。入力のコントラストが変われば同じ閾値でもエッジ画素率が桁で動き、極端な場合は 0 になる。ならば閾値ではなく**密度を揃えればよい**。
...
ここに本レポートを通じた見方が現れている。条件の強さは、増やすほど良い資源ではない。第 4.2 節と第 5 節で見たのは資源が足りない側の破綻、本節で見たのは資源が過剰な側の代償である。密度が低い入力でも scale を上げれば構造整合は改善するが（synth_sparse は 0.333→0.877）、条件地図が空になった入力では増幅しても空間的な指定は戻らない。ただし両者を同時に振っていないので交互作用そのものは測っていない。**密度と scale は、条件の実効的な強さという一つの量に対する別々の支払い方である。** だから最適値は入力の中ではなく、入力の構造と何を達成したいかの組み合わせの中にある。
...
[5] Stéfan van der Walt, Johannes L. Schönberger, Juan Nunez-Iglesias, et al. "scikit-image: image processing in Python." *PeerJ* 2:e453, 2014. DOI: 10.7717/peerj.453 （Crossref で照合。同梱データを入力画像に使用、帰属は LICENSE_DATA.md）
