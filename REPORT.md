...
diffusers 0.31.0 では slicing の有無で attention の実装が入れ替わる。既定は `F.scaled_dot_product_attention`、slicing 有効時はスコア行列を明示的に構成する `Attention.get_attention_scores` を通り、後者は加算元を未初期化のまま確保している。
...
加算元を差し替えると挙動が変わる。**`torch.empty` を `torch.zeros` にするだけで NaN が消えた。** 粒度 1・4・auto のいずれでも消え、潜在変数の絶対値の最大は 3.379〜3.383 と、slicing 無効時の 3.377 に近い値へ戻る（経路が違うので厳密には一致しない）。シードを変えても消えたままだった。

中身も直接測った（`exp10_buffer_content.py`）。本体と同じ手順のまま `baddbmm` の直前に `isfinite` をかける。fp16 では 2944 回のうち 2847 回のバッファが非有限値を含み、要素数では 90.3% に達する。fp32 は 2944 回すべて 0 個だった。生のビット列を見ると大半が `0x7E00`、fp16 の正準的な NaN である。ランダムなゴミではなく、解放されたメモリに残っていた NaN がそのまま再利用されている。**非有限値を含むバッファが最初に現れた呼び出しと、出力が最初に NaN になった呼び出しは同じである。** IEEE 754 では 0 × NaN も 0 × Inf も NaN なので、`beta = 0` は非有限値を無害化しない。**原因は加算元の未初期化バッファであり、規約が保証するはずの「参照されない」が MPS では成り立っていない。** ただし最初にバッファが汚染された時点では、まだどの出力も NaN になっていない。その NaN がどこから来たのかは追えていない。
...
問題は、この設定が推奨されている点にある。diffusers の MPS 最適化に関する公式文書は RAM 64GB 未満で `enable_attention_slicing()` の有効化を案内しており、本環境の 16GB はその条件に当てはまる。案内どおりに従うと生成が全滅し、しかも失敗が静かなのでユーザーは自分の設定ミスを疑う。あわせて、生成直後に NaN と単色を検出して例外を投げる検査を組み込んだ。静かに壊れる失敗は下流のすべての指標を無意味にする。
...
**環境に固有の可能性がある。** 第 4.1 節の NaN は M1 / PyTorch 2.2.2 / MPS / fp16 で観測した。加算元の中身が結果に伝播することは差し替えで確かめたが、中身そのものは観測しておらず、fp32 で露見しない理由も分かっていない。他の PyTorch 版やハードウェアで再現するかも未確認である。推奨設定に従うと例外を出さずに壊れるという構図そのものは、環境が違っても起こりうる。
