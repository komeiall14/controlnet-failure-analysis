...
条件が失われたときに起きるのは、残差が消えることでも一様な項になることでもなく、**条件地図に由来する成分が縮むこと**である。本節で測ったのは残差だけなので、縮んだ残差が出力にどう表れるかはここでは言えない。制御が効かなくなる理由は、注入される信号の大きさそのものが落ちる点に求められる。
...
[5] Stéfan van der Walt, Johannes L. Schönberger, Juan Nunez-Iglesias, et al. "scikit-image: image processing in Python." *PeerJ* 2:e453, 2014. DOI: 10.7717/peerj.453 （入力画像に同梱データを使用。帰属は LICENSE_DATA.md。全著者を Crossref で照合済み）

書誌情報は [1] [2] [5] を Crossref で DOI 照合し、[1] は DBLP と Semantic Scholar でも確認した（DBLP 上で arXiv 版と会議録版が別項目で登録されており、本会議採録である）。[1] の supplementary は CVF の公開版を参照した。
