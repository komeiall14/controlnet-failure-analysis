...
中身も直接測った（`exp10_buffer_content.py`）。本体と同じ手順のまま `baddbmm` の直前に `isfinite` をかける。fp16 では 2944 回のうち 2847 回のバッファが非有限値を含み、要素数では 90.3% に達する。fp32 は 2944 回すべて 0 個だった。しかも**非有限値を含むバッファが最初に現れた呼び出しと、出力が最初に NaN になった呼び出しは同じ 64 回目**である。IEEE 754 では 0 × NaN も 0 × Inf も NaN なので、`beta = 0` は非有限値に対して無力である。**原因は加算元の未初期化バッファであり、規約が保証するはずの「参照されない」が MPS では成り立っていない。** なぜ fp16 でだけ非有限値が乗るのかは分かっていない。ランダムなビット列なら fp16 で 1/32、fp32 で 1/256 のはずで、実測の 90% 対 0% は合わない。メモリに何が残っているかまでは追っていない。
