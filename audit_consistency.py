...
提出するのはレポート（PDF）とソースコードの両方で、採点者はどちらも見る。
本文だけを直してコードの docstring や README を放置すると、リポジトリを
開いた時点で食い違いが見える。実際、held-out の画像数が
本文 7 / exp9 の docstring 8 / dataset の docstring 11 / LICENSE 11 と
四つに割れた。目で追うのは無理なので機械で照合する。
