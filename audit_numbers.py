...
本文に数値を書く以上、その一つずつに出所が要る。目視では見落とすので、
本文中の数値を全部抜き出し、次のいずれかに当たるかを確かめる。
...
ALLOW = {
    ...
    # 書誌・識別子。実験データではないので照合の対象にならない
    "48266454": "学籍番号", "2023": "ICCV の開催年／参考文献の発行年",
    "3813": "ControlNet 論文の開始ページ", "3824": "同 終了ページ",
    "10.1109": "DOI の登録機関コード", "51070.2023": "DOI の会議コード",
    "00355": "DOI の論文番号", "844": "diffusers の該当行（開始）",
    "851": "同（終了）", "224": "macOS のビルド番号 23E224 の一部",
    "4.44": "transformers 4.44.2 の版の一部", "4.11": "OpenCV 4.11.0 の版の一部",
    "0.22.0": "scikit-image のバージョン", "1.16.0": "SciPy のバージョン",
    "7": "入力数", "9": "掃引段数", "15": "画像数", "45": "条件数",
    "60": "条件数", "90": "対の数", "70": "点数", "135": "順伝播回数",
    "83": "重複除去後の件数", "68": "重複除去後の件数",
    "28": "論文の Figure 番号", "2.1": "supplementary の節番号",
    "4.2": "論文の節番号（アブレーション）", "3.4": "論文の節番号",
    "754": "IEEE 754（規格の名前）", "65504": "fp16 の表現できる最大値（規格の定数）",
    "32": "fp16 の指数部5ビットから 2^5（非有限になる割合の分母）",
    "256": "fp32 の指数部8ビットから 2^8（同上）",
