"""開発に使っていない画像で、第6節の改善をそのまま試す。

第6節の改善と、その適用条件（改善前の密度が目標を下回るときだけ適用する）は
real_images() の10点と合成5点から作った。同じデータで評価すれば性能の
見積もりは楽観側に寄る。そこで開発に使っていない skimage.data の画像を別に取り、
規則を一切調整せずに当てて再現するかを見る（点数は dataset.holdout_images に従う）。
