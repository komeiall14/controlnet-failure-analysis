...
| # | 失敗条件 | 要点 |
|---|---|---|
| 1 | 推奨されている省メモリ設定が沈黙したまま出力を壊す | `enable_attention_slicing()` を有効にすると UNet 順伝播で NaN。例外も警告も出ず黒画像が返る。原因は MPS の `baddbmm` が「`beta=0` なら加算元を参照しない」という BLAS の規約を守らず 0×C を実際に計算する点。diffusers が加算元を `torch.empty` で渡すのは規約に従った実装で、そこに残る非有限値が 0×NaN=NaN として出てくる。CPU では規約が守られる。利用者側は `torch.zeros` に変えるだけで回避できる |
...
**改善**: (1) 上記 `torch.empty` → `torch.zeros` の修正。20ステップの本番条件で、slicing を有効にしたまま 5 枚すべてが正常に生成され、slicing 無効時との画素差は平均 0.18（0〜255階調）。(2) 目標密度への二分探索 + CLAHE、密度連動 scale。どちらも拡散の前段なので生成のステップ数は変わらない。
修正版は slicing 無効時より確保量のピークが 2 割少なく、省メモリ効果を保ったまま出力だけが直る。
低コントラスト側で構造整合が改善し（Δ中央値 +0.0665、共通参照）、プロンプト追従には有意な変化が出ない。
...
図を描き直すなら追加で `pip install matplotlib`。ただし本文の図1（`fig_camera_sweep.png`）だけは
生成画像が要るので、無い環境では既存の PNG をそのまま残して描き直さない。
...
本文に対応する記述は無い（そう表示される）。生成画像（545 枚・168MB、リポジトリに含めない）が必要な箇所は
`results/image_metrics.csv` と `results/exp7_policy.csv` に測定値があり、画像の有無で
同じ値になることを確認済み（出力を差分して、出所の表示行以外は完全一致）。
実行時にどちらから計算したかを表示する。
...
21 段ある。各段は `results/<名前>.done` を作り、再実行すると完了済みの段は
...
| `exp14_amplify_empty.py` | 空の条件地図で scale を上げても指定が戻らないかを掃引で確かめる |
...
| `audit_consistency.py` | 提出物どうしの一貫性を点検する（画像数・共通の数値・ファイルと図の一覧・名指し） |
...
| `check_health.py` | 生成画像の健全性を一括検査。第4.1節の証拠としてわざと壊してある exp12 の5枚は別勘定にする |
...
| `results/exp13_memory.csv` | 3条件の生成中の確保量のピーク（第4.1節の 1.165 / 0.905 GiB の出所） |
| `results/exp14_amplify_empty.csv` | 空の条件地図での scale 掃引（第8節の出所） |
